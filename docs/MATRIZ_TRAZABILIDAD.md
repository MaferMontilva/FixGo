...
| 11 | Oportunidades compatibles (categoría+zona) | `GET /professionals/me/opportunities[/:id]` | `ProfessionalOpportunitiesPage` | PROFESSIONAL | ✅ |
...
| 19 | Ver valoraciones recibidas | `GET /reviews/mine`, `GET /reviews/professional/:id` | `ProfessionalReviewsPage` | PROFESSIONAL/público | ✅ |
...
- **Seguridad**: todos los endpoints sensibles protegidos con `JwtAuthGuard` + `RolesGuard` y `@Roles` correctos; identidad desde el JWT; el registro no puede crear ADMIN; sin fugas de datos entre usuarios (validación de propiedad en cada flujo).
